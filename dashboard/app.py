...
def display_validated_leads(
    ...
    for index, lead in enumerate(leads, start=1):
        ...
        # Draft key keyed on source_url, falling back to index if a lead
        # has no URL, so drafts persist correctly across reruns.
        draft_key = f"email_draft_{lead.get('source_url') or index}"

        is_expanded = index == 1 or bool(
        ...
        with st.expander(
            ...
            # --- Email generation (ported from the email-drafting branch) ---
...
# NOTE: this block only stores the result in session_state; it never
# renders results directly. Results are rendered below from
# session_state on every rerun (including reruns triggered by the
# "Generate Email" button), so email drafts don't wipe the scan.
if generate_clicked:
    payload = {
