...
def discover_potential_leads(payload: dict) -> dict:
    response = requests.post(DISCOVER_POTENTIAL_LEADS_URL, json=payload, timeout=1800)
    if response.status_code != 200:
...
def display_manual_review(manual_review: list[dict], developer_mode: bool) -> None:
    ...
    llm_items = [i for i in manual_review if i.get("review_type") == "llm"]
    other_items = [i for i in manual_review if i.get("review_type") not in {"similarity", "llm"}]
    rc1, rc2, rc3 = st.columns(3)
    rc1.metric("Similarity Review", len(similarity_items))
    rc2.metric("LLM Review", len(llm_items))
...
# ---- Tab 4: LinkedIn Profile Analyzer ----
with tabs[3]:
