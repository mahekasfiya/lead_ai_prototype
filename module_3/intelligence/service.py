...
# MERGE NOTES
# -----------------------------------------------------------------------------
# Base: the branch with generate_email_draft() / _template_email() /
# _build_contact_text() and the llm_model constructor param, since that's
# what powers email generation.
#
# Swapped in from the other branch: build_report() now takes a mandatory
# `deadline: DeadlineAssessment` argument, and all the scoring/urgency/
# buying-stage/risk/recommendation logic reasons about deadline.status
# ("active" / "unknown" / "expired") instead of the coarser
# qualification.requirement_status enum. This is a real behavior change:
# `pursue` is now gated on the deadline being verified as active, not just
# on score + a service match.
#
# IMPORTANT: build_report() no longer has a default for `deadline`, so the
# call site in discovery_service.py must pass one. See the follow-up patch
# applied there.
# =============================================================================
...
class LeadIntelligenceService:
    ...
    def build_report(
        ...
        urgency = self._get_urgency(deadline.status)
        ...
        temporal_status_verified = deadline.status == "active"
        pursue = (
        ...
        return LeadIntelligenceReport(
            ...
            metadata={
                ...
                "deadline_status": deadline.status,
                "deadline": (
                    deadline.deadline.isoformat() if deadline.deadline else None
                ),
...
"""
    ...
    def _calculate_opportunity_score(
        ...
        if deadline_status == "active":
            reasons.append("The submission deadline or active status was verified.")

        if deadline_status == "unknown":
            reasons.append(
                "The opportunity appears relevant, but its current "
                "submission status is unverified."
            )
    ...
    def _get_urgency(self, deadline_status: str) -> str:
        if deadline_status == "active":
            return "high"
    ...
    def _get_buying_stage(
        ...
        document_type = qualification.document_type.value
        ...
        if deadline_status == "active":
            ...
            if document_type in {"eoi", "partner_request"}:
    ...
    def _build_business_need(
        ...
        if lead.summary and top_service_name:
            return (
                f"{lead.summary} The strongest identified Triway "
    ...
    def _build_risks(
        ...
        if deadline.status == "expired":
            risks.append(
                "The submission deadline has passed or the opportunity "
                "is no longer active."
            )

        if deadline.status == "unknown":
            risks.append(
                "The requirement appears genuine, but its current "
                "submission status or deadline has not been verified."
            )

        if deadline.status == "active" and deadline.deadline is None:
            risks.append(
                "The opportunity appears active based on page language, "
                "but no explicit dated deadline was found."
            )
    ...
    def _recommended_action(
        ...
        if deadline.status == "expired":
            return (
                "Do not assign this opportunity to sales because the "
                "submission deadline has passed or the opportunity is "
                "no longer active."
            )

        if deadline.status == "unknown":
            return (
                "Verify the submission deadline and current procurement "
                "status before assigning the lead to business development."
            )
        ...
        if priority == "critical":
            return (
                "Assign immediately to business development and begin "
                "buyer-contact research."
            )

        if priority == "high":
            return (
                "Validate the procurement details and contact the buyer "
    ...
    def _build_talking_points(
        ...
        if lead.country:
            points.append(
                f"Confirm delivery coverage and references for {lead.country}."
            )
